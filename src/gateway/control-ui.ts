...
function serveResolvedIndexHtml(res: ServerResponse, body: string, opts?: { autoToken?: string }) {
  let html = body;
  if (opts?.autoToken) {
    // Inject a tiny bootstrap script that seeds the gateway token into
    // sessionStorage so the Control UI connects automatically.  This is
    // gated on dangerouslyDisableDeviceAuth + allowInsecureAuth in the
    // caller, so it only fires for explicitly open deployments.
    const escaped = opts.autoToken.replace(/[\\'"]/g, "\\$&");
    // Use a CSP nonce so the inline script is allowed by the Content-Security-Policy.
    const nonce = randomBytes(16).toString("base64");
    // The UI derives the default gateway URL as `ws[s]://<host>[<basePath>]`
    // and uses `openclaw.control.token.v1:<scope>` as the sessionStorage key.
    const script = [
      `<script nonce="${nonce}">try{`,
      `var p=location.protocol==="https:"?"wss":"ws";`,
      `var k="openclaw.control.token.v1:"+p+"://"+location.host;`,
      `sessionStorage.setItem(k,"${escaped}")`,
      "}catch(e){}</script>",
    ].join("");
    html = html.replace("</head>", `${script}</head>`);
    // Override the CSP header to include the nonce for the injected script.
    const csp = buildControlUiCspHeader().replace(
      "script-src 'self'",
...
export function handleControlUiHttpRequest(
  ...
  // Resolve auto-token for open deployments (dangerouslyDisableDeviceAuth +
  // allowInsecureAuth both enabled).  The token is injected into the HTML so
  // the Control UI connects without manual token entry.
  const controlUiCfg = opts?.config?.gateway?.controlUi;
  const autoToken =
    controlUiCfg?.dangerouslyDisableDeviceAuth === true &&
    controlUiCfg?.allowInsecureAuth === true &&
    typeof opts?.config?.gateway?.auth?.token === "string" &&
    opts.config.gateway.auth.token.length > 0
      ? opts.config.gateway.auth.token
      : undefined;
  const indexHtmlOpts = autoToken ? { autoToken } : undefined;
  ...
  if (safeFile) {
    try {
      ...
      if (path.basename(safeFile.path) === "index.html") {
        serveResolvedIndexHtml(res, fs.readFileSync(safeFile.fd, "utf8"), indexHtmlOpts);
        return true;
      }
  ...
  if (safeIndex) {
    try {
      ...
      serveResolvedIndexHtml(res, fs.readFileSync(safeIndex.fd, "utf8"), indexHtmlOpts);
      return true;
    } finally {
